# CITE: https://github.com/russel0719/UNet-3-Plus-Pytorch/blob/main/models/unet3plus.py
...
class UNet3Plus(nn.Module):
    def __init__(self,
        ...
        Args:
            input_shape (list): Input shape of the model [channels, height, width].
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        # Classification Guided Module
        if self.CGM:
            assert self.cgm is not None, "Classification Guided Module is not enabled."
            cls = self.cgm(e5)
        ...
        if self.deep_supervision:
            assert self.deep_sup is not None, "Deep supervision is not enabled."
            outputs.extend([
...
if __name__ == "__main__":
    unet_3P = UNet3Plus(input_shape=[1, 320, 320],
                        output_channels=4)
