...
class mgam_Patched_Mha(mgam_BaseSegDataset):
    """Dataset of 3D patches in MHA format generated by itk_patch."""
    def __init__(self, *args, **kwargs) -> None:
        # img_suffix and seg_map_suffix define image/label file naming
        super().__init__(img_suffix='_image.mha', seg_map_suffix='_label.mha', *args, **kwargs)

    def _split(self):
        # list all case folders
        all_series = [d for d in os.listdir(self.data_root)
                      if os.path.isdir(os.path.join(self.data_root, d))]
        # sort by numeric order if possible
        def sort_key(x):
            m = re.search(r"\d+", x)
            return int(m.group()) if m else x
        all_series = sorted(all_series, key=sort_key)
        total = len(all_series)
        t0 = int(total * self.SPLIT_RATIO[0])
        t1 = t0 + int(total * self.SPLIT_RATIO[1])
        if self.split == 'train':
            return all_series[:t0]
        elif self.split == 'val':
            return all_series[t0:t1]
        elif self.split == 'test':
            return all_series[t1:]
        else:
            raise RuntimeError(f"Unsupported split: {self.split}")
        ...
        for series in self._split():
            ...
            for fname in sorted(os.listdir(series_folder)):
                if fname.endswith(self.img_suffix):
                    img_path = os.path.join(series_folder, fname)
                    lbl_name = fname.replace(self.img_suffix, self.seg_map_suffix)
                    lbl_path = os.path.join(series_folder, lbl_name)
                    if not os.path.exists(lbl_path):
                        print_log(f"Label not found: {lbl_path}", MMLogger.get_current_instance(), logging.WARN)
                        continue
