...
class LUNA16_formatter(StandardFileFormatter):
    ...
    def tasks(self) -> list:
        labels_folder = os.path.join(self.data_root, 'seg-lungs-LUNA16')
        labels_list = [file
                       for file in os.listdir(labels_folder) 
                       if file.endswith('.mhd')]
        subset_image_folder_list = ["subset{}".format(i) for i in range(10)]
        images_list = [os.path.join(self.data_root, subset_image_folder, file)
                       for subset_image_folder in subset_image_folder_list
                       for file in os.listdir(os.path.join(self.data_root, subset_image_folder))
                       if file.endswith('.mhd')]
        ...
        for image_path in images_list:
            ...
            if series_file_name in labels_list:
                ...
                task_list.append((image_path, label_path, self.dest_root, series_name, self.spacing, self.size))
        
        return task_list

if __name__ == "__main__":
    formatter = LUNA16_formatter.start_from_argparse()
    formatter.execute()
