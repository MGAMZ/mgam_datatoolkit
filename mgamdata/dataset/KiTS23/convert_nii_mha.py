...
def convert_one_case(args):
    ...
    series_nii_label_path = series_nii_image_path.replace('imaging', 'segmentation')
    # 构建路径，保持文件存储结构不变
    series_id = re.search(r'case_(\d+)',
                          os.path.dirname(series_nii_image_path)).group(1)
    ...
    # 类分离的标注文件合并后保存
    input_image_mha = convert_nii_sitk(series_nii_image_path, dtype=np.int16, nii_fdata_order='zyx')
    input_label_mha = convert_nii_sitk(series_nii_label_path, dtype=np.uint8, nii_fdata_order='zyx')
...
def convert_and_save_nii_to_mha(input_dir: str, 
    ...
    for series_name in os.listdir(input_dir):
        if series_name.startswith('case_'):
            series_nii_image_path = os.path.join(input_dir, series_name, 'imaging.nii.gz')
