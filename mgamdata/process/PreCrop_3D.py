...
class PreCropper3D:
    ...
    def crop_per_series(self, args:tuple):
        cropper, image_itk_path, anno_itk_path, num_cropped, save_folder = args
        cropper: RandomCrop3D
        os.makedirs(save_folder, exist_ok=True)
        
        for crop_idx, (img_array, anno_array) in enumerate(
                self.Crop3D(cropper, image_itk_path, anno_itk_path, num_cropped)
            ):
