...
class MedNeXt(object):
    ...
    def predict_from_img(self, x: Union[torch.Tensor, np.ndarray]):
        ...
        x = x.numpy().astype('float32')[None, None, :, :]
        x = self._window_norm(x)
        infer_output = self._triton_infer_ndarray(x)
        infer_output = torch.from_numpy(infer_output).permute(1,0,2,3)

        self._unload_model()
        return infer_output # (Class, 1, H, W)


    def predict_whole_series(self, x: np.ndarray):
        # 检查输入
        ...
        for i in tqdm(range(D), desc='序列推理中', leave=False, dynamic_ncols=True, disable=not self.show_pbar):
            ...
            infer_result = torch.from_numpy(infer_result).to(device='cpu', non_blocking=True)
    ...
    def predict_from_dcm(self, dcm_series_folder_path:str) -> Tuple[sitk.Image, sitk.Image]:
        ...
        pred_index = series_result.argmax(dim=0).numpy().astype(np.uint8) # (D, H, W)
...
if __name__ == "__main__":
    ...
    # 50例测试推理
    test_samples_folder = '/fileser51/zhangyiqin.sx/Sarcopenia_Data/Test_7986/dcm'
    save_folder = '/fileser51/zhangyiqin.sx/DeployAndEngineering/Test/240930Test50Cases'
