...
from predict_from_raw_data_trt import nnUNetPredictor
from triton_server_client_rengji2d import MedNeXt, LoadDcmAsSitkImage_EngineeringOrder
...
def kmeans_segmentation(img, mask_pred):
    ...
    return [psoas_muscle_nodule_point_set,      # 腰大肌肌肉
...
def ct_sarcopenia_predict(seriesUid: str, seriesPath: str):
    ...
    # 调用函数并打印结果
    config_data = parse_cfg_file('/fileser/rongkuan/python_dev/alpha/config/main_config.cfg')
    print(config_data)
    ...
    try:
        ...
        return PredictionResponse(code=1006, msg = 'nnUNet Init Failed.')
    ...
    try:
        ...
        return PredictionResponse(code=1004, msg = 'fail to seg')

    kmeans_results = kmeans_segmentation(img_raw, mask_pred)
...
async def predict(request: PredictionRequest):
    ...
    if not (isinstance(seriesPath,str) and isinstance(seriesUid,str)):
        return PredictionResponse(code=1002, msg = 'type of para is wrong')
