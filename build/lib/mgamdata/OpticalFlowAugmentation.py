...
import numpy as np
import torch
from cv2.cuda import bilateralFilter, fastNlMeansDenoising
...
from .OpenCV_PyTorch_Conversion import Mat2Tensor, Tensor2Mat
...
class BroxOpticalFlow_LabelAugment(OpticalFlow_BaseLabelAugment):
    def __init__(self, gpu_id:int=0, use_mp:bool=False, *args, **kwargs):
        ...
        self.OF = cv2.cuda.BroxOpticalFlow.create(
            ...
            solver_iterations=4)
        self.RoI_HistNorm_Mask = self.create_circle_in_square(self.H, self.H//3)
    ...
    def Denoise(images:np.ndarray, method:str) -> np.ndarray:
        if method.lower() == 'wavelet':
            ...
            denoise = lambda x: bilateralFilter(
